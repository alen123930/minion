...
legion/
├── pom.xml                      # Maven 父 POM
├── contracts/                   # legion-contracts：协议 DTO + 事件类型枚举 + reason code
│                                 #    server/daemon 共用；前端类型由此手抄对齐（或后续 codegen）
├── server/                      # legion-server：Spring Boot API server（WebMvc + SSE/WS + MyBatis + Flyway）
├── daemon/                      # legion-daemon：Spring Boot 应用（无 web 容器，仅 actuator）
│   └── src/main/java/io/legion/daemon/
